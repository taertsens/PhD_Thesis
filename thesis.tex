\documentclass[showinstructions,faculty=firw,department=bwk,phddegree=bwk]{adsphd}
...
%%% COVER: Settings %%%
\title{Hybrid Numerical-Physical Modelling of Multidirectional Waves and Currents}
%\subtitle{Multiscale computing for Dummies}

\author{Tim}{Aertsens}

\supervisor{Prof.~dr.~ir.~J.~Monbaliu}{}
\supervisor{Prof.~dr.~A.~Toffoli}{}
\president{Prof.~dr.~ir.~The~Chairman}{}
\jurymember{Prof.~dr.~ir.~P.~Rauwoens}{KU Leuven}
\jurymember{Prof.~dr.~ir.~D.~Huybrechs}{KU Leuven}
\jurymember{Prof.~dr.~I.~Young}{UniMelb}
\externaljurymember{Prof.~dr.~M.~Streicher}{UGent}
\externaljurymember{Prof.~dr.~G.~Ducrozet}{EC Nantes}

% Your research group within the department
% e.g. iMinds-DistriNet, Scientific Computing Group, ...
\researchgroup{Hydraulics and Geotechnics}
\website{https://bwk.kuleuven.be/hydr} % Leave empty to hide
\email{tim.aertsens@kuleuven.be} % Leave empty to hide

\address{Kasteelpark Arenberg 40}
...
\includechapter{wavemaking}
\includechapter{wavebreaking}
\includechapter{wci}
\includechapter{wsi}
